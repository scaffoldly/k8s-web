/**
 * Integration test for @k8s-web/angular
 * Tests listing namespaces in a real Angular application
 */
import { test, expect } from '@playwright/test';

test.describe('Angular Integration Tests - Namespaces', () => {
  test.beforeEach(async ({ page }) => {
    // Navigate to the simple test page
    await page.goto('http://localhost:4200/test-simple.html');
  });

  test('should load the test page', async ({ page }) => {
    await expect(page.locator('h1')).toContainText('Angular Integration Test');
    await expect(page.locator('h2')).toContainText('@k8s-web/angular');
  });
